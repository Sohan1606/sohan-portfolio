...
const PIPELINE = [
  ...
  { step: "09", label: "Iterate",      description: "The system is never finished — only at a different stage of being improved."   },
];
...
const PHASES = [
  ...
  },
  {
    ...
    description: "Built multi-agent AI systems through hackathon work — a 5-agent adversarial pipeline for bias detection. Learned agent orchestration and LLM integration under constraints.",
...
interface PipelineStepProps {
  step: string; label: string; description: string; isLast: boolean;
}
  ...
  <div className="flex gap-4">
    <div className="flex flex-col items-center shrink-0">
      <div className="flex items-center justify-center w-6 h-6
                      border border-border/40 bg-surface shrink-0">
        <span className="font-mono text-[0.45rem] text-signal tracking-widest">{step}</span>
      </div>
      {!isLast && (
        <div className="w-px flex-1 bg-border/20 mt-1 min-h-[1.75rem]"
          aria-hidden="true" />
      )}
    </div>
    <div className={`flex flex-col gap-1 ${isLast ? "pb-0" : "pb-5 md:pb-6"}`}>
      <span className="font-sans font-semibold text-fog/80 text-sm tracking-tight">
        {label}
      </span>
      <p className="font-sans text-fog/40 font-light text-sm leading-relaxed max-w-[28ch]">
        {description}
      </p>
    </div>
  </div>
  ...
  <div className={`grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6
    ...
    <div className="md:col-span-3">
      <div className="flex items-baseline gap-3">
        <span className={`font-mono text-[0.45rem] tracking-widest
                          ${phase.current ? "text-signal" : "text-muted/30"}`}>
          {phase.number}
        </span>
        <span className={`font-sans font-black text-xl md:text-2xl
                          tracking-tighter leading-none
                          ${phase.current ? "text-fog" : "text-fog/45"}`}>
          {phase.label}
        </span>
      </div>
      <span className={`font-mono text-[0.45rem] tracking-widest uppercase block mt-1.5
                        ${phase.current ? "text-signal/70" : "text-muted/30"}`}>
        {phase.area}
      </span>
      {phase.current && (
        <div className="mt-2.5 inline-flex items-center gap-1.5">
          <span className="w-[3px] h-[3px] rounded-full bg-signal animate-blink"
            aria-hidden="true" />
    ...
    <div className="md:col-span-5 md:col-start-4">
      <p className={`font-sans font-light leading-relaxed text-sm md:text-base
                     ${phase.current ? "text-fog/70" : "text-fog/38"}`}>
        {phase.description}
      </p>
    </div>
    <div className="md:col-span-3 md:col-start-10 flex flex-wrap content-start gap-x-3 gap-y-1.5">
      {phase.signals.map(s => (
        <span key={s} className={`font-mono text-[0.45rem] tracking-wide
                                  ${phase.current ? "text-fog/50" : "text-muted/28"}`}>
          {s}
        </span>
      ))}
    </div>
    ...
    <section id="journey" aria-label="Engineering path and process"
      ...
      <div className="editorial py-section">
        ...
        <motion.div variants={reveal} initial="hidden" whileInView="show" viewport={VP}
          ...
          <p className="font-sans text-fog/50 font-light leading-relaxed
        ...
        <div className="mt-14 md:mt-20 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
          <div className="lg:col-span-6">
            <motion.div variants={stagger} initial="hidden" whileInView="show"
              viewport={VP_CLOSE} className="flex flex-col">
          ...
          <div className="lg:col-span-5 lg:col-start-8">
            ...
            <motion.div variants={reveal} initial="hidden" whileInView="show"
              viewport={VP_CLOSE} className="mt-8 pt-6 border-t border-border/[0.15]">
        ...
        <motion.div variants={reveal} initial="hidden" whileInView="show" viewport={VP}
          ...
          <p className="font-sans text-fog/50 font-light leading-relaxed
            ...
            Each phase built on the last. Phase 07 is already in progress.
          </p>
        </motion.div>
        ...
        <motion.div variants={reveal} initial="hidden" whileInView="show" viewport={VP}
          ...
          <div className="lg:col-span-2 pt-1">
            <span className="font-mono text-[0.45rem] text-muted/35
                             tracking-widest uppercase">Direction</span>
          </div>
